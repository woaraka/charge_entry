<?php
    ...
    $_SESSION['header'] = 'Report (Work Order)';
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <div class="dropdown">
              <button class="btn btn-info dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Work Order Reports
              </button>
          ...
          <div>
           <form method="post" action="work-order-report.php">
          ...
          <div>
            <?php
                echo '<br>';
                $list = mysqli_query($con,"SELECT f_id, date, p_name, mrn, prob FROM workorder_problem WHERE c_date>= '$from_date' && c_date<= '$to_date'");
				while(list($fac,$date, $P_name, $mrn, $problem) = mysqli_fetch_array($list))
                {
                    $date = date("m/d/Y",strtotime($date));
                    $query = mysqli_query($con,"SELECT nick_name FROM facility WHERE f_id = '$fac'");
                    list($fname)=mysqli_fetch_array($query);
                        echo
                        "<span style='font-size:16px; color:black;text-transform: uppercase;'>$date--$fname--$P_name--MRN--$mrn<br>$problem<br><br></span>";
                }
              ?>
